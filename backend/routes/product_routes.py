    ...
    try:
        ...
        query = """
            SELECT p.product_id, p.product_code, p.product_name,
                   ...
                   p.selling_price, p.reorder_level, p.warehouse_reorder_level,
        ...
        for row in rows:
            products.append({
                ...
                "reorder_level": row[6],
                "warehouse_reorder_level": row[7],
                "status": row[8],
                "description": row[9],
                "has_image": row[10],
                "product_image": get_product_image_url(row[0], row[10]),
                "category_status": row[11],
...
def admin_add_product():
    ...
    try:
        ...
        warehouse_reorder_level = payload.get("warehouse_reorder_level")
        ...
        warehouse_reorder_level = parse_non_negative_int(
        ...
        cur.execute("""
            INSERT INTO product (
                ...
                reorder_level, warehouse_reorder_level, status, description,
...
def update_product_status(product_id, status, actor_user_id):
    ...
    try:
        ...
        action = "ACTIVATE_PRODUCT" if is_active else "INACTIVE_PRODUCT"
        ...
        log_audit(
            ...
            "Product Management",
            product_id,
            f"{state_text.capitalize()} product {product[0]}."
        )

        return jsonify({"message": f"Product {state_text} successfully"}), 200

    except Exception as e:
        if conn:
            conn.rollback()
        print(f"ERROR product status update {status}:", e)
        return jsonify({"message": str(e)}), 500
...
def admin_update_product_reorder_level(product_id):
    ...
    try:
        ...
        log_audit(
            ...
            "Product Management",
            product_id,
            f"Updated product {product[0]} reorder level to {reorder_level}."
        )
        cur.close()
        conn.close()
